         ...
         <div className="flex-1 h-full min-h-0 overflow-y-auto bg-slate-900/30 rounded-2xl border border-slate-800/50 p-4 lg:p-6 custom-scrollbar">
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 pb-12">
               ...
               <UnitCard 
                  ...
                  desc="视频音频的核心标准。加入了 K-weighting 加权（模拟人耳对高频更敏感）和门限控制（自动忽略静音段）。"
               ...
               <UnitCard 
                  ...
                  desc="通过 4倍过采样 (Oversampling) 预测采样点之间的模拟波形峰值。普通 Peak 表可能会漏掉采样点间的‘隐形’峰值。"
                  highlight="它是最后的“保险丝”"
               ...
               <UnitCard 
                  ...
                  desc="相对变化的单位。数值上 1 LU = 1 dB。它用于描述“距离达标还需要调大多少”。"
...
const Visualizer: React.FC<{ mode: 'LUFS' | 'DBTP' | 'LU' }> = ({ mode }) => {
   ...
   useEffect(() => {
      ...
      const render = () => {
         ...
         const targetY = mode === 'DBTP' ? logicalH * 0.3 : logicalH * 0.5; 
         ...
         if (mode === 'DBTP') {
            ...
            for(let x=0; x<=logicalW; x++) {
               const spikeX = (waveSpeed) % (logicalW + 300) - 150;
               const dist = Math.abs(x - spikeX);
               let val = 0;
               if (dist < 100) {
                  val = Math.max(0, 1 - dist/40) * Math.cos(dist*0.12);
               }
               const y = centerY - val * (logicalH*0.4); 
            ...
            for(let x=0; x<=logicalW; x+=sampleSpacing) {
               const spikeX = (waveSpeed) % (logicalW + 300) - 150;
               const dist = Math.abs(x - spikeX);
               let val = 0;
               if (dist < 100) {
         ...
         if (mode === 'LU') {
            ...
            ctx.beginPath();
            ctx.moveTo(logicalW/2 - gap, targetY);
            ctx.lineTo(logicalW/2 + gap, targetY);
            ctx.moveTo(logicalW/2 + gap, currentH);
            ctx.lineTo(logicalW/2 + gap + barW, currentH); 
            ctx.stroke();
