   ...
   return (
      <div className="flex flex-col lg:flex-row h-full">
         ...
         <div className="flex-1 bg-zinc-50 dark:bg-zinc-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
            ...
            {/* Dynamic Range Chart */}
            <div className="w-full max-w-2xl bg-zinc-50/50 dark:bg-zinc-800 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 relative">
               <div className="absolute top-4 left-6 text-xs font-bold text-zinc-600 dark:text-zinc-400 flex items-center gap-2">
                  ...
                  <svg className="w-full h-full overflow-visible" viewBox={`0 0 ${width} ${height}`}>
                     <defs>
                        <linearGradient id="gradCurve" x1="0" y1="0" x2="0" y2="1">
                           <stop offset="0%" stopColor="oklch(82% 0.16 85)" stopOpacity="0.5"/>
                           <stop offset="100%" stopColor="oklch(82% 0.16 85)" stopOpacity="0"/>
                        </linearGradient>
                     </defs>
                     ...
                     {[15, 11.5, 8].map((val, i) => {
                        ...
                        return (
                           <g key={val}>
                              <line x1={paddingX} y1={y} x2={width - paddingX} y2={y} stroke="oklch(44% 0.017 286)" strokeWidth="1" strokeDasharray="4,4" opacity="0.5" />
                              <text x={paddingX - 10} y={y + 4} textAnchor="end" fontSize="10" fill="oklch(62% 0.015 286)" font-family="monospace">{val} Stops</text>
                           </g>
                        );
                     })}

                     {/* The Curve */}
                     <polyline points={points} fill="none" stroke="oklch(82% 0.16 85)" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round"/>
                     
                     {/* Area under curve */}
                     <polygon points={`${paddingX},${height-paddingY} ${points} ${width-paddingX},${height-paddingY}`} fill="url(#gradCurve)" opacity="0.2"/>

                      {/* Active Indicator */}
                      <circle cx={activeX} cy={activeY} r="6" fill="oklch(82% 0.16 85)" stroke="white" strokeWidth="2"/>
                     ...
                     {isoSteps.map((s, i) => {
                        ...
                        if (i === 0 || i === 3 || i === 8 || i === 11) {
                           const x = paddingX + (i / (isoSteps.length - 1)) * graphW;
                           return (
                              <text key={s} x={x} y={height - 5} textAnchor="middle" fontSize="10" fill="oklch(62% 0.015 286)" font-family="monospace">{s}</text>
                           );
                        }
                        return null;
                     })}
                     ...
                     <g transform={`translate(${paddingX + (3 / 11) * graphW}, 30)`}>
                        <text textAnchor="middle" fill="oklch(68% 0.18 155)" fontSize="10" fontWeight="bold" dy="-5">Base 800</text>
                        <path d="M0,0 L0,10" stroke="oklch(68% 0.18 155)" strokeWidth="1" opacity="0.5"/>
                     </g>
                     <g transform={`translate(${paddingX + (8 / 11) * graphW}, 30)`}>
                        <text textAnchor="middle" fill="oklch(76% 0.18 85)" fontSize="10" fontWeight="bold" dy="-5">Base 12800</text>
                        <path d="M0,0 L0,10" stroke="oklch(76% 0.18 85)" strokeWidth="1" opacity="0.5"/>
                     </g>
                  </svg>
               </div>
            </div>
            ...
            <div className="flex gap-4 mt-8 w-full max-w-2xl">
               <div className="flex-1 bg-zinc-50 dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                  <span className="text-xs text-zinc-600 dark:text-zinc-400">当前 ISO</span>
                   <span className={`text-xl font-mono font-bold ${iso >= 12800 ? 'text-yellow-500 dark:text-yellow-400' : 'text-zinc-900 dark:text-white'}`}>{iso}</span>
               </div>
               <div className="flex-1 bg-zinc-50 dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                  <span className="text-xs text-zinc-600 dark:text-zinc-400">动态范围</span>
                  <span className="text-xl font-mono font-bold text-primary-500 dark:text-cyan-400">{currentDR.toFixed(1)} Stops</span>
               </div>
            </div>
         </div>
         ...
         <div className="w-full lg:w-96 bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 p-6 flex flex-col overflow-y-auto">
            <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-6 flex items-center gap-2"><Zap size={20} className="text-yellow-500"/> 双原生 ISO 原理</h3>
            ...
            <div className="mb-8">
               <Slider 
                  ...
                  unit=""
               />
               <div className="flex justify-between text-[11px] text-zinc-500 dark:text-zinc-400 font-mono px-1">
                  <span>100</span>
                  <span>800</span>
                  <span>12800</span>
                  <span>102400</span>
            ...
            <div className="space-y-6">
               <div className="bg-zinc-50 dark:bg-zinc-800 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800">
                  ...
                  <p className="text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed text-justify mb-3">
               ...
               <div className="bg-zinc-50 dark:bg-zinc-800 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800">
                  <h4 className="text-sm font-bold text-zinc-900 dark:text-white mb-3">实战建议 (Pro Tips)</h4>
                  <ul className="text-xs text-zinc-600 dark:text-zinc-400 space-y-2 list-disc pl-4">
                     <li>
                        <strong className="text-zinc-900 dark:text-white">锁定基准：</strong> 尽量只使用 <span className="text-emerald-400">ISO 800</span> 和 <span className="text-yellow-400">ISO 12800</span> 这两档拍摄 S-Log3。
                     </li>
                     <li>
                        <strong className="text-zinc-900 dark:text-white">避免中间值：</strong> ISO 10000 是最差的档位（第一电路的极限），噪点最多。往上拨一档到 12800，画质反而瞬间变好。
                     </li>
                     <li>
                        <strong className="text-zinc-900 dark:text-white">ND 滤镜：</strong> 12800 太亮怎么办？不要降 ISO，请使用 ND 滤镜压暗光线，以保留珍贵的动态范围。
                     </li>
                  </ul>
               </div>
            </div>
