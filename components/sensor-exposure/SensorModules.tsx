  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      <div className="flex-1 bg-zinc-50 dark:bg-black relative overflow-hidden flex items-center justify-center">
         <div 
            className="absolute inset-0 bg-cover bg-center transition-colors duration-200 ease-out" 
            style={{ 
         ...
         {/* HUD Overlay */}
         <div className="absolute top-4 right-4 bg-zinc-50/80 dark:bg-black backdrop-blur-md p-3 rounded-lg border border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-white shadow-md">
            <div className="text-[10px] text-zinc-600 dark:text-zinc-400 font-bold uppercase tracking-wider mb-1">Exposure Value</div>
      ...
      <div className="w-full lg:w-80 bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 p-6 flex flex-col overflow-y-auto">
        <h3 className="text-lg font-bold mb-6 text-zinc-900 dark:text-white flex items-center gap-2"><Sun size={18} className="text-primary-400"/> 曝光三要素</h3>
        ...
        <div className="mt-auto pt-6 text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
...
export const GrayCardModule: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      <div className="flex-1 bg-zinc-50 dark:bg-black flex flex-col items-center justify-center p-4 lg:p-8 relative overflow-hidden">
         <div className="w-full max-w-sm aspect-square border-8 border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 flex flex-col items-center justify-center relative shadow-lg overflow-hidden rounded-lg">
            ...
            <div className="absolute inset-0 pointer-events-none">
               ...
               {meteringMode === 'SPOT' && (
                  <div className="w-full h-full flex items-center justify-center">
                     <div className="w-12 h-12 rounded-full border-2 border-green-500 bg-transparent shadow-[0_0_20px_oklch(68%_0.18_155_/_0.5)] flex items-center justify-center">
                        <div className="w-1 h-1 bg-green-500"></div>
                     </div>
                  </div>
               )}
               {meteringMode === 'HIGHLIGHT' && (
                  <div className="absolute top-4 right-4 flex items-center gap-2 bg-red-900/80 px-2 py-1 rounded text-xs text-red-700 dark:text-red-200 border border-red-500/50">
            ...
            {/* Histogram Mini-HUD */}
            <div className="absolute bottom-0 left-0 right-0 h-12 bg-zinc-100/60 dark:bg-zinc-950 backdrop-blur-sm border-t border-zinc-200 dark:border-zinc-800 flex flex-col justify-center px-4">
               <div className="w-full h-2 bg-zinc-100 dark:bg-zinc-700 rounded-full overflow-hidden relative">
                  <div className="absolute top-0 bottom-0 w-1 bg-zinc-50 shadow-[0_0_10px_white] transition-colors duration-300" style={{ left: `${histPos}%` }}></div>
                  <div className="absolute top-0 bottom-0 left-[46%] w-0.5 bg-green-500/50"></div> {/* 18% Mark */}
               </div>
               <div className="flex justify-between text-[11px] text-zinc-500 dark:text-zinc-400 mt-1 font-mono">
                  <span>0 (Black)</span>
      ...
      <div className="w-full lg:w-96 bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 p-6 flex flex-col overflow-y-auto">
        <h3 className="text-lg font-bold mb-6 flex items-center gap-2 text-zinc-900 dark:text-white"><Target size={18} className="text-cyan-400"/> 测光与灰卡</h3>
        
        <div className="space-y-6">
           <div>
              <div className="text-xs font-bold text-zinc-600 dark:text-zinc-400 mb-2">测光模式</div>
              <div className="grid grid-cols-2 gap-2">
                 {['MULTI', 'CENTER', 'SPOT', 'HIGHLIGHT'].map(m => (
                    <button key={m} onClick={()=>setMeteringMode(m as any)} className={`p-2 rounded border text-xs font-bold transition-colors ${meteringMode===m ? 'bg-cyan-900/50 border-cyan-500 text-white' : 'border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:border-zinc-300 dark:hover:border-zinc-600'}`}>
                       {m}
                    </button>
                 ))}
              </div>
           ...
           <div>
              <div className="text-xs font-bold text-zinc-600 dark:text-zinc-400 mb-2">拍摄对象材质</div>
              <div className="flex gap-2 bg-zinc-50 dark:bg-zinc-800 p-1 rounded-lg">
                 {['SNOW', 'GRAY', 'COAL'].map(s => (
                    <button key={s} onClick={() => setScene(s as any)} className={`flex-1 py-2 text-xs rounded font-bold transition-colors ${scene === s ? 'bg-zinc-600 text-zinc-900 dark:text-white shadow' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-300'}`}>
        ...
        <div className="mt-6 bg-zinc-50 dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800">
           <h4 className="text-xs font-bold text-primary-500 dark:text-cyan-400 mb-2">为什么拍雪会变灰？</h4>
           <p className="text-[10px] text-zinc-600 dark:text-zinc-400 leading-relaxed">
              相机的测光表是“傻”的，它默认把看到的一切都还原为 <span className="text-zinc-900 dark:text-white">18% 中性灰</span>。
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      <div className="flex-1 bg-zinc-50 dark:bg-black flex items-center justify-center p-8 relative overflow-hidden">
         ...
         <div className="w-full max-w-[500px] aspect-square relative flex items-center justify-center">
             <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-lg">
               ...
               <circle cx="50" cy="50" r="48" fill="oklch(13% 0.006 286)" stroke="oklch(33% 0.015 286)" strokeWidth="0.5" />
               <text x="50" y="8" textAnchor="middle" fontSize="3" fill="oklch(44% 0.017 286)">Lens Image Circle</text>

               {/* Full Frame Ref (Faint) */}
               <rect x="14" y="26" width="72" height="48" fill="none" stroke="oklch(27% 0.006 286)" strokeWidth="0.5" strokeDasharray="2,2" />
      ...
      <div className="w-full lg:w-80 bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 p-6 flex flex-col">
         <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6 flex items-center gap-2"><BoxSelect size={18} className="text-cyan-400"/> 画幅对比</h3>
            ...
            {Object.entries(specs).map(([k, v]) => (
                <button key={k} onClick={() => setFormat(k as any)} className={`w-full p-4 rounded-lg border text-left transition-colors group ${format === k ? 'bg-zinc-100 dark:bg-zinc-800 border-zinc-500' : 'border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700'}`}>
                   <div className="flex justify-between items-center mb-1">
                      <span className={`font-bold text-sm ${format === k ? 'text-zinc-900 dark:text-white' : 'text-zinc-600 dark:text-zinc-400'}`}>{v.name}</span>
                     <span className="text-[10px] bg-zinc-50/80 dark:bg-black px-2 py-0.5 rounded text-zinc-500 dark:text-zinc-400">{v.crop}x</span>
                  </div>
                  <div className="w-full bg-zinc-50 dark:bg-zinc-950 h-1.5 rounded-full overflow-hidden mt-2">
                     <div className={`h-full transition-colors ${k==='FF'?'bg-white':k==='APSC'?'bg-yellow-400':'bg-red-400'}`} style={{width: `${(1/v.crop)*100}%`}}></div>
                  </div>
               </button>
            ))}
         </div>
         <div className="mt-auto bg-zinc-50 dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800 text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
            <strong className="text-zinc-900 dark:text-white block mb-1">等效系数原理：</strong>
            <br/>
...
export const RollingShutterModule: React.FC = () => {
  ...
  useEffect(() => {
     ...
     const loop = () => {
        ...
        ctx.fillStyle = 'oklch(8% 0.005 286)'; // slate-950
        ...
        for(let i=0; i<CONSTANTS.ROLLING_SHUTTER_SCANLINES; i++) {
           ...
           ctx.fillStyle = 'oklch(63% 0.21 25)'; 
           // Propeller blade
  ...
  return (
     <div className="flex flex-col lg:flex-row h-full">
        <div ref={containerRef} className="flex-1 bg-zinc-50 dark:bg-black flex items-center justify-center p-4 relative overflow-hidden">
           <canvas ref={canvasRef} className="block w-full h-full rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-800" />
           <div className="absolute top-4 left-4 text-xs font-mono text-zinc-500 dark:text-zinc-400 bg-zinc-950/50 px-2 rounded">
        ...
        <div className="w-full lg:w-80 bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 p-6">
           <h3 className="text-lg font-bold mb-6 text-zinc-900 dark:text-white flex items-center gap-2"><ScanLine size={18} className="text-red-400"/> 卷帘快门畸变</h3>
           ...
           <div className="space-y-3">
              <div className="p-3 bg-zinc-50 dark:bg-zinc-800 rounded border border-zinc-200 dark:border-zinc-800">
                 <div className="text-xs font-bold text-zinc-900 dark:text-white mb-1">什么是果冻效应？</div>
                 <p className="text-[10px] text-zinc-600 dark:text-zinc-400 leading-relaxed">
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      <div className="flex-1 bg-zinc-50 dark:bg-zinc-900 flex flex-col items-center justify-center p-8 relative">
         <div className="absolute top-8 right-8 w-20 h-20 rounded-full border-4 border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800 shadow-lg flex items-center justify-center z-10">
            <div className="text-3xl font-black text-zinc-900 dark:text-white">{mode}</div>
            <div className="absolute -bottom-2 text-[11px] text-zinc-500 dark:text-zinc-400 uppercase tracking-widest bg-zinc-50 dark:bg-zinc-900 px-1">Mode</div>
         </div>
         ...
         <div className="relative w-full max-w-xl aspect-video bg-zinc-50 dark:bg-black rounded-xl overflow-hidden border-4 border-zinc-200 dark:border-zinc-800 shadow-lg">
            <div className="absolute inset-0 bg-cover bg-center transition-colors duration-300" style={{
            ...
            <div className="absolute inset-0 p-4 flex flex-col justify-between pointer-events-none">
               ...
               <div className="flex justify-between items-end text-zinc-900 dark:text-white font-mono text-sm drop-shadow-md bg-zinc-950/20 p-2 rounded">
                  ...
                  <span className="text-zinc-700 dark:text-zinc-300">{evComp > 0 ? '+' : ''}{evComp} EV</span>
               </div>
            </div>
         </div>
         
         <div className="mt-8 text-center max-w-md bg-zinc-50/50 dark:bg-zinc-800 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800">
      ...
      <div className="w-full lg:w-80 bg-zinc-50 dark:bg-zinc-800 p-6 flex flex-col border-l border-zinc-200 dark:border-zinc-800 select-none overflow-y-auto">
         <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6 flex items-center gap-2"><Camera size={18} className="text-primary-400"/> 拍摄模式</h3>
         <div className="grid grid-cols-4 gap-2 mb-8">
            {['P', 'A', 'S', 'M'].map((m) => (
               <button key={m} onClick={() => setMode(m as any)} className={`aspect-square rounded-xl font-black text-lg border-2 flex items-center justify-center transition-colors ${mode === m ? 'bg-cyan-600 border-cyan-400 text-zinc-900 dark:text-white shadow-lg scale-105' : 'bg-zinc-700 border-zinc-300 dark:border-zinc-600 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-600'}`}>{m}</button>
            ))}
         </div>
         <div className="space-y-6">
            <div className={`transition-opacity ${mode === 'S' || mode === 'P' ? 'opacity-50 pointer-events-none' : ''}`}>
               <div className="flex justify-between mb-1"><span className="text-xs font-bold text-zinc-700 dark:text-zinc-300">光圈 (F-Stop)</span>{(mode==='S'||mode==='P') && <Lock size={10} className="text-yellow-500"/>}</div>
               <input type="range" min="1.4" max="22" step="0.1" value={aperture} onChange={e => setAperture(parseFloat(e.target.value))} className="w-full accent-cyan-500 h-1 bg-zinc-600 rounded appearance-none" />
            </div>
            <div className={`transition-opacity ${mode === 'A' || mode === 'P' ? 'opacity-50 pointer-events-none' : ''}`}>
               <div className="flex justify-between mb-1"><span className="text-xs font-bold text-zinc-700 dark:text-zinc-300">快门 (Shutter)</span>{(mode==='A'||mode==='P') && <Lock size={10} className="text-yellow-500"/>}</div>
               <input type="range" min="10" max="2000" step="10" value={shutter} onChange={e => setShutter(parseFloat(e.target.value))} className="w-full accent-cyan-500 h-1 bg-zinc-600 rounded appearance-none" />
            </div>
            <div>
               <div className="flex justify-between mb-1"><span className="text-xs font-bold text-zinc-700 dark:text-zinc-300">ISO 感光度</span></div>
               <input type="range" min="100" max="12800" step="100" value={iso} onChange={e => setIso(parseFloat(e.target.value))} className="w-full accent-emerald-500 h-1 bg-zinc-600 rounded appearance-none" />
            </div>
            {mode !== 'M' && (
