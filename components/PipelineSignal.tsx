      ...
      <div className="flex-1 bg-zinc-50 dark:bg-zinc-900 flex flex-col items-center justify-center p-8">
        ...
        <div className="flex gap-8 items-center flex-wrap justify-center">
          ...
          <div className="flex flex-col items-center gap-2">
             <div className="text-xs text-zinc-600 dark:text-zinc-400">8K 传感器 (33MP)</div>
             <div className="grid grid-cols-4 gap-0.5 bg-zinc-50 dark:bg-zinc-800 p-1 border border-zinc-200 dark:border-zinc-800">
                {[...Array(16)].map((_, i) => (
                   <div key={i} className={`w-8 h-8 flex items-center justify-center text-xs ${
                      ...
                      mode === 'BINNING' ? 'bg-zinc-700 text-zinc-600 dark:text-zinc-400 opacity-30' : 
          ...
          <div className="flex flex-col items-center gap-1">
             <div className="text-[10px] text-zinc-500 dark:text-zinc-400 font-mono">Process</div>
             <div className={`h-1 w-20 rounded ${mode === 'OVERSAMPLING' ? 'bg-gradient-to-r from-cyan-600 to-emerald-400' : 'bg-zinc-600'}`}></div>
          </div>

          {/* Output Video */}
          <div className="flex flex-col items-center gap-2">
             <div className="text-xs text-zinc-600 dark:text-zinc-400">4K 视频 (8MP)</div>
             <div className="grid grid-cols-2 gap-0.5 bg-zinc-50 dark:bg-zinc-800 p-1 border border-zinc-200 dark:border-zinc-800">
                {[...Array(4)].map((_, i) => (
                   <div key={i} className="w-16 h-16 bg-emerald-500 flex items-center justify-center text-zinc-900 dark:text-white text-xs font-bold border border-emerald-400/50">
                      Pixel
                   </div>
                ))}
             </div>
        ...
        <div className="mt-12 flex gap-8">
           <div className="text-center">
              ...
              <div className="text-sm font-bold text-zinc-700 dark:text-zinc-300">细节表现</div>
           </div>
        </div>
   ...
   return (
      <div className="flex flex-col lg:flex-row h-full">
         <div className="flex-1 bg-zinc-50 dark:bg-zinc-900 flex flex-col items-center justify-center p-4">
            ...
            <div className="relative border-4 border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-black w-[400px] h-[300px] flex items-center justify-center overflow-hidden">
               {/* Base Text */}
               ...
               <div className="absolute bottom-10 right-10 text-6xl font-bold text-primary-600 z-10 mix-blend-screen" style={{
               ...
               <div className="absolute inset-0 grid grid-cols-2">
                  <div className="bg-zinc-50/[0.05] border-r border-white/10"></div>
                  <div className="bg-zinc-50/[0.05]"></div>
               </div>
            </div>
            ...
            <div className="mt-8 flex gap-4">
               <div className="flex flex-col items-center">
                  <div className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">Luma (Y)</div>
                  <div className="grid grid-cols-4 w-16 h-16 border border-zinc-600">
                     {[...Array(16)].map((_,i) => <div key={i} className="bg-gray-200 border border-gray-400/50"></div>)}
                  </div>
               </div>
               <div className="flex flex-col items-center">
                  <div className="text-xs text-zinc-500 dark:text-zinc-400 mb-2">Chroma (UV)</div>
                  <div className="grid grid-cols-4 w-16 h-16 border border-zinc-600">
                     {subsampling === '444' && [...Array(16)].map((_,i) => <div key={i} className="bg-primary-500 border border-blue-600/50"></div>)}
                     {subsampling === '422' && [...Array(8)].map((_,i) => <div key={i} className="bg-primary-500 col-span-2 border border-blue-600/50"></div>)}
                     {subsampling === '420' && [...Array(4)].map((_,i) => <div key={i} className="bg-primary-500 col-span-2 row-span-2 border border-blue-600/50"></div>)}
                  </div>
               </div>
            </div>
         ...
         <div className="w-full lg:w-80 bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 p-6 flex flex-col">
            <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6 flex items-center gap-2"><Layers size={18}/> 色彩采样</h3>
            
            <div className="space-y-4">
               <button onClick={() => setSubsampling('444')} className={`w-full p-3 rounded border text-left ${subsampling === '444' ? 'bg-cyan-900 border-cyan-500 text-white' : 'border-zinc-700 text-zinc-600 dark:text-zinc-400'}`}>
                  <div className="font-bold text-sm">4:4:4 (全采样)</div>
                  <div className="text-[10px] opacity-70">每个像素都有独立的亮度和色彩信息。电影级画质，文件极大。</div>
               </button>
               <button onClick={() => setSubsampling('422')} className={`w-full p-3 rounded border text-left ${subsampling === '422' ? 'bg-zinc-100 dark:bg-zinc-800 border-white text-white' : 'border-zinc-700 text-zinc-600 dark:text-zinc-400'}`}>
               ...
               <button onClick={() => setSubsampling('420')} className={`w-full p-3 rounded border text-left ${subsampling === '420' ? 'bg-zinc-100 dark:bg-zinc-800 border-zinc-500 text-white' : 'border-zinc-700 text-zinc-600 dark:text-zinc-400'}`}>
   ...
   return (
      <div className="flex flex-col lg:flex-row h-full">
         <div className="flex-1 bg-zinc-50 dark:bg-black flex flex-col items-center justify-center p-8">
            {/* Display Visualizer */}
            <div className="w-full max-w-2xl aspect-video bg-zinc-50 dark:bg-zinc-900 relative border border-zinc-200 dark:border-zinc-800 overflow-hidden">
               {/* Gradient Sky */}
               ...
               <div className="absolute bottom-4 left-4 text-zinc-900 dark:text-white text-xs font-mono drop-shadow-md">
         ...
         <div className="w-full lg:w-80 bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 p-6 flex flex-col">
            <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6 flex items-center gap-2"><Binary size={18}/> 色彩深度 (Bit Depth)</h3>

            <div className="space-y-4">
               <button onClick={() => setBitDepth('8bit')} className={`w-full p-3 rounded border text-left transition-colors ${bitDepth === '8bit' ? 'bg-zinc-100 dark:bg-zinc-800 border-primary-500 shadow' : 'border-zinc-700 text-zinc-600 dark:text-zinc-400'}`}>
                  <div className="font-bold text-sm text-primary-600 dark:text-primary-400 mb-1">8-bit (1670万色)</div>
               ...
               <button onClick={() => setBitDepth('10bit')} className={`w-full p-3 rounded border text-left transition-colors ${bitDepth === '10bit' ? 'bg-zinc-100 dark:bg-zinc-800 border-cyan-500 shadow' : 'border-zinc-700 text-zinc-600 dark:text-zinc-400'}`}>
                  <div className="font-bold text-sm text-primary-500 dark:text-cyan-400 mb-1">10-bit (10.7亿色)</div>
                  <div className="text-[10px] text-zinc-600 dark:text-zinc-400">
