      ...
      <div className="flex flex-col lg:flex-row h-full overflow-hidden">
         <div className="flex-1 bg-zinc-50 dark:bg-black relative flex items-center justify-center overflow-hidden">
            ...
            <div className="absolute top-4 right-4 pointer-events-none">
               <div className={`w-24 h-24 rounded-full border-4 shadow-lg flex items-center justify-center backdrop-blur-sm transition-all
                  ${filterType === 'CPL' ? 'border-blue-400 bg-primary-900/20' : 
                    filterType === 'ND' ? 'border-zinc-600 bg-zinc-950/60' : 
                    filterType === 'MIST' ? 'border-white/40 bg-zinc-50/[0.05]' : 'border-zinc-800 bg-transparent'}
               `}>
                  <Disc size={32} className={filterType === 'NONE' ? 'text-zinc-800 dark:text-zinc-200' : 'text-white/20'} />
               </div>
            </div>
         </div>
         ...
         <div className="w-full lg:w-80 bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 p-6 flex flex-col overflow-y-auto no-scrollbar">
            ...
            <div className="grid grid-cols-2 gap-2 mb-6">
               {(['NONE', 'CPL', 'ND', 'MIST'] as const).map(t => (
                  <button key={t} onClick={() => setFilterType(t)} className={`p-3 rounded border text-xs font-bold transition-colors ${filterType === t ? 'bg-cyan-900/50 border-cyan-400 text-cyan-100 shadow-inner' : 'border-zinc-700 text-zinc-500 dark:text-zinc-400 hover:border-zinc-500'}`}>
                     {t === 'NONE' ? '无滤镜' : t}
                  </button>
               ))}
            </div>

            <div className="min-h-[160px] bg-zinc-50 dark:bg-zinc-800 rounded-lg p-4 border border-zinc-200 dark:border-zinc-800">
               {filterType === 'NONE' && <p className="text-xs text-zinc-600 dark:text-zinc-400">物理滤镜安装在镜头前端。它们在光线接触传感器之前改变光的物理性质，这是后期数字处理无法完美替代的。</p>}
               {filterType === 'CPL' && (
                  <>
                     <Slider label="偏振旋转" value={rotation} min={0} max={90} unit="°" onChange={setRotation} />
                     <p className="text-[10px] text-zinc-600 dark:text-zinc-400 mt-2 leading-relaxed italic">原理：选择性吸收非金属表面的偏振反射光。可消除水面反光、增加蓝天饱和度。</p>
                  </>
               )}
               {filterType === 'ND' && (
                  <>
                     <Slider label="减光档位" value={ndStop} min={1} max={10} unit=" Stops" onChange={setNdStop} />
                     <p className="text-[10px] text-zinc-600 dark:text-zinc-400 mt-2 leading-relaxed italic">原理：中性密度玻璃，均匀减少进光量。用于在强光下使用大光圈（虚化）或慢速快门（流水）。</p>
                  </>
               )}
               {filterType === 'MIST' && (
                  <>
                     <Slider label="柔光强度" value={mistStrength} min={0} max={1} step={0.1} onChange={setMistStrength} />
                     <p className="text-[10px] text-zinc-600 dark:text-zinc-400 mt-2 leading-relaxed italic">原理：分散的高光在阴影处溢出。用于软化数码锐度，营造电影感梦幻光晕。</p>
                  </>
               )}
            </div>
         </div>
