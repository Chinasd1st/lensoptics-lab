...
export const QuizIntro: React.FC<QuizIntroProps> = ({ totalQuestions, onStart, onImport, onOpenEditor, onDownloadCsv, onOpenBuilder }) => {
  ...
  return (
    <div className="max-w-3xl w-full text-center space-y-8 animate-fade-in zoom-in duration-500 relative">
      {/* Header Buttons */}
      <div className="absolute top-0 right-0 flex gap-2">
         ...
         <button 
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-zinc-50 dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white rounded-lg text-xs font-bold transition-colors border border-zinc-200 dark:border-zinc-800"
            title="导入 JSON 试卷"
         >
         ...
         <button 
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-zinc-50 dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white rounded-lg text-xs font-bold transition-colors border border-zinc-200 dark:border-zinc-800"
            title="导出题库 CSV"
         >
         ...
         <button 
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-zinc-50 dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-primary-500 dark:text-primary-400 hover:text-primary-600 dark:hover:text-primary-300 rounded-lg text-xs font-bold transition-colors border border-zinc-200 dark:border-zinc-800"
            title="打开编辑器"
         >
      ...
      <div className="w-32 h-32 bg-zinc-50 dark:bg-zinc-900 rounded-full flex items-center justify-center mx-auto border-4 border-zinc-200 dark:border-zinc-800 shadow-lg relative">
         <Brain size={64} className="text-primary-400" />
         <div className="absolute -top-2 -right-2 bg-yellow-500 text-black text-xs font-bold px-3 py-1 rounded-full border-2 border-zinc-900">
            PRO
         </div>
      </div>
      ...
      <div>
        ...
        <p className="text-zinc-600 dark:text-zinc-400 text-lg max-w-lg mx-auto leading-relaxed">
          当前题库共有 <span className="text-zinc-900 dark:text-white font-bold">{totalQuestions}</span> 道精选题。
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
         ...
         <div className="bg-zinc-100 dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800">
            <div className="text-xs font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-widest mb-4 flex items-center justify-center gap-2">
         ...
         <div className="bg-zinc-100 dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800">
            ...
            <div className="space-y-4">
               <div className="flex items-center gap-3">
                  <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 w-12 text-right">EASY</span>
                  <input type="range" min="0" max="100" step="10" value={diffDist.easy} onChange={(e)=>handleDistChange('easy', parseInt(e.target.value))} className="flex-1 accent-emerald-500 h-1.5 bg-zinc-100 dark:bg-zinc-700 rounded-full appearance-none"/>
                  <span className="text-xs font-mono w-8 text-right">{diffDist.easy}%</span>
               </div>
               <div className="flex items-center gap-3">
                  <span className="text-xs font-bold text-primary-500 dark:text-primary-400 w-12 text-right">MED</span>
                  <input type="range" min="0" max="100" step="10" value={diffDist.medium} onChange={(e)=>handleDistChange('medium', parseInt(e.target.value))} className="flex-1 accent-primary-500 h-1.5 bg-zinc-100 dark:bg-zinc-700 rounded-full appearance-none"/>
                  <span className="text-xs font-mono w-8 text-right">{diffDist.medium}%</span>
               </div>
               <div className="flex items-center gap-3">
                  <span className="text-xs font-bold text-red-600 dark:text-red-400 w-12 text-right">HARD</span>
                  <input type="range" min="0" max="100" step="10" value={diffDist.hard} onChange={(e)=>handleDistChange('hard', parseInt(e.target.value))} className="flex-1 accent-red-500 h-1.5 bg-zinc-100 dark:bg-zinc-700 rounded-full appearance-none"/>
                  <span className="text-xs font-mono w-8 text-right">{diffDist.hard}%</span>
               </div>
               <div className="text-[10px] text-zinc-500 dark:text-zinc-400 text-right italic">
                  *系统会尽量接近所选比例
               </div>
            </div>
         </div>
