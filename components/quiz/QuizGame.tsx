...
export const QuizGame: React.FC<QuizGameProps> = ({ questions, onFinish }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative overflow-hidden">
      {/* Top Bar (Fixed) */}
       <div className="w-full max-w-3xl mx-auto pt-4 pb-4 px-4 shrink-0 z-10 bg-zinc-50/80 dark:bg-zinc-950 backdrop-blur-sm sticky top-0">
         <div className="flex justify-between items-end mb-2">
            <div className="text-sm font-bold text-zinc-600 dark:text-zinc-400">
               Question <span className="text-zinc-900 dark:text-white text-xl">{currentIndex + 1}</span><span className="text-zinc-600 dark:text-zinc-400">/{questions.length}</span>
            </div>
            <div className={`flex items-center gap-2 font-mono text-xl font-bold ${timeLeft < 10 ? 'text-red-500 animate-pulse' : 'text-zinc-700 dark:text-zinc-200'}`}>
         ...
         <div className="flex w-full gap-[2px] h-1.5 rounded-full overflow-hidden" role="progressbar" aria-valuenow={currentIndex + 1} aria-valuemin={1} aria-valuemax={questions.length} aria-label="Quiz progress">
            {questions.map((_, i) => {
               ...
               if (i === currentIndex) color = 'bg-zinc-900 animate-pulse dark:bg-white'; 
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar w-full">
         <div className="min-h-full flex flex-col justify-center items-center p-4 pb-20">
            <div className="w-full max-w-3xl animate-slide-in-right duration-300" key={currentIndex}>
               
               <div className="mb-6">
                  <div className="flex items-center gap-2 mb-3">
                     <span className="inline-block px-3 py-1 text-xs font-bold tracking-wider text-primary-500 dark:text-primary-400 uppercase bg-primary-900/20 rounded-full border border-primary-900/50">{currentQ.category}</span>
                     <span className={`inline-block px-3 py-1 text-xs font-bold tracking-wider uppercase rounded-full border ${currentQ.difficulty === 'EASY' ? 'text-emerald-400 bg-emerald-900/20 border-emerald-900/50' : currentQ.difficulty === 'MEDIUM' ? 'text-primary-400 bg-primary-900/20 border-primary-900/50' : 'text-red-400 bg-red-900/20 border-red-900/50'}`}>{currentQ.difficulty}</span>
                     <span className="inline-block px-3 py-1 text-xs font-bold tracking-wider text-primary-600 dark:text-primary-400 uppercase bg-primary-900/20 rounded-full border border-purple-900/50">{currentQ.type}</span>
                  </div>
                  <h2 className="text-xl md:text-2xl font-bold text-zinc-900 dark:text-white leading-tight">{currentQ.question}</h2>
               </div>
               ...
               {isAnswerRevealed && (
                  <div className="bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-600 p-4 rounded-xl animate-fade-in slide-in-from-bottom-4 backdrop-blur-md">
                     <div className="flex items-center justify-between mb-3 border-b border-zinc-200 dark:border-zinc-800 pb-2">
                        <div className="flex items-center gap-2">
                           ...
                           {earnedPoints > 0 && earnedPoints < 10 && <span className="text-[10px] bg-yellow-900/30 text-yellow-700 dark:text-yellow-200 px-1.5 py-0.5 rounded border border-yellow-500/20">部分得分</span>}
                        </div>
                        <div className="text-[10px] text-zinc-500 dark:text-zinc-400 flex items-center gap-1"><Keyboard size={12}/> Press Enter for Next</div>
                     </div>
                     <div className="flex items-start gap-3">
                        <HelpCircle className="text-primary-400 shrink-0 mt-0.5" size={16}/>
                        <div>
                           <h4 className="font-bold text-zinc-900 dark:text-white text-xs mb-1">技术解析</h4>
