  ...
  return (
    <div className="flex flex-col lg:flex-row h-full">
      ...
      <div className="flex-1 bg-zinc-50 dark:bg-zinc-900 flex flex-col items-center justify-center p-8 relative">
         {/* Mode Dial Visual */}
         <div className="absolute top-8 right-8 w-24 h-24 rounded-full border-4 border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800 shadow-lg flex items-center justify-center transform rotate-12 transition-all">
            <div className="absolute inset-0 rounded-full border border-zinc-300 dark:border-zinc-600 border-dashed opacity-20"></div>
         ...
         <div className="relative w-full max-w-xl aspect-video bg-zinc-50 dark:bg-black rounded-lg overflow-hidden border-8 border-zinc-200 dark:border-zinc-800 shadow-inner">
            ...
            {(() => {
               ...
               return (
                  <>
                     <div className="absolute inset-0 bg-cover bg-center transition-colors duration-200" style={{
                     ...
                     {/* HUD */}
                     <div className="absolute bottom-0 w-full h-12 bg-zinc-950/80 flex items-center justify-around text-primary-500 dark:text-cyan-400 font-mono text-xs px-4">
                        <div className={`flex flex-col items-center ${mode === 'S' || mode === 'P' ? 'text-yellow-400' : ''}`}>
                           <span className="text-[11px] text-zinc-500 dark:text-zinc-400">IRIS</span>
                           <span className="font-bold">f/{aperture.toFixed(1)}</span>
                        </div>
                        <div className={`flex flex-col items-center ${mode === 'A' || mode === 'P' ? 'text-yellow-400' : ''}`}>
                           <span className="text-[11px] text-zinc-500 dark:text-zinc-400">SHUTTER</span>
                           <span className="font-bold">1/{Math.round(shutter)}</span>
                        </div>
                        <div className="flex flex-col items-center">
                           <span className="text-[11px] text-zinc-500 dark:text-zinc-400">ISO</span>
                           <span className="font-bold">{iso}</span>
                        </div>
                        <div className="flex flex-col items-center border-l border-zinc-200 dark:border-zinc-800 pl-4">
                           <span className="text-[11px] text-zinc-500 dark:text-zinc-400">M.M.</span>
      ...
      {/* Control Panel */}
      <div className="w-full lg:w-80 bg-zinc-50 dark:bg-zinc-800 p-6 flex flex-col border-l border-zinc-200 dark:border-zinc-800 select-none">
         <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-6 flex items-center gap-2"><Camera size={18}/> 模式转盘</h3>
         ...
         <div className="grid grid-cols-4 gap-2 mb-8">
            {['P', 'A', 'S', 'M'].map((m) => (
               <button 
                  ...
                  className={`aspect-square rounded-full font-black text-lg border-2 transition-colors hover:scale-110 flex items-center justify-center
                     ${mode === m ? 'bg-cyan-600 border-cyan-400 text-zinc-900 dark:text-white shadow-[0_0_15px_rgba(8,145,178,0.5)]' : 'bg-zinc-700 border-zinc-300 dark:border-zinc-600 text-zinc-600 dark:text-zinc-400'}
                  `}
               >
                  {m}
               </button>
