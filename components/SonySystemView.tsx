      ...
      <div className="flex flex-col lg:flex-row h-full">
         <div className="flex-1 bg-black relative flex items-center justify-center overflow-hidden">
            ...
            <div className="absolute bottom-8 left-8 bg-slate-900/90 border border-slate-700 p-4 rounded-lg shadow-xl backdrop-blur-md">
               ...
               <div className="w-48 h-1 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full transition-all duration-500 ${enabled ? 'bg-emerald-500 w-full' : 'bg-red-500 w-1/4'}`}></div></div>
            </div>
         </div>
         <div className="w-full lg:w-96 bg-slate-900 border-l border-slate-800 p-6 flex flex-col overflow-y-auto">
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2"><Sparkles size={20} className="text-orange-500"/> GM 光学技术</h3>
            <div className="space-y-6">
               ...
               <div className="flex items-center justify-between bg-slate-800 p-3 rounded border border-slate-700">
                  ...
                  <button onClick={() => setEnabled(!enabled)} className={`w-12 h-6 rounded-full transition-colors relative ${enabled ? 'bg-cyan-600' : 'bg-slate-600'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${enabled ? 'left-7' : 'left-1'}`}></div></button>
               </div>
               <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
                  {techType === 'XA' ? (
                     <div className="animate-in fade-in slide-in-from-right-4">
                        ...
                        <p className="text-xs text-slate-400 leading-relaxed text-justify mb-2"><strong className="text-white">作用：</strong> 修正球差（Spherical Aberration）。</p>
                        <p className="text-xs text-slate-400 leading-relaxed text-justify">普通非球面镜片表面研磨精度较低，会导致焦外光斑出现同心圆纹路（洋葱圈）。Sony XA 镜片精度高达 <strong>0.01微米</strong>，不仅让光线完美汇聚（高锐度），还确保了焦外如奶油般顺滑。</p>
                     </div>
                  ) : (
                     <div className="animate-in fade-in slide-in-from-right-4">
                        <h4 className="text-sm font-bold text-green-400 mb-2 flex items-center gap-2"><Triangle size={14}/> 低色散 (ED)</h4>
                        <p className="text-xs text-slate-400 leading-relaxed text-justify mb-2"><strong className="text-white">作用：</strong> 修正色差（Chromatic Aberration）。</p>
                        <p className="text-xs text-slate-400 leading-relaxed text-justify">不同颜色的光波长不同，通过玻璃时的折射率也不同（像三棱镜一样散开）。普通玻璃会导致高反差边缘出现紫边/绿边。ED 镜片具有反常色散特性，能强行将红绿蓝三色光拉回到同一个焦点。</p>
                     </div>
                  )}
               </div>
            </div>
   ...
   <div className="p-8 h-full overflow-y-auto">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
         <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
            ...
            <p className="text-xs text-slate-400 leading-relaxed"><span className="text-emerald-400 font-bold">相位检测 (PDAF)</span> 覆盖画面 90% 以上区域。结合反差检测，既快又准。<br/>“实时追踪”功能会利用颜色、距离（深度）、图案（亮度）来死死咬住被摄体。</p>
         </div>
         <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
            ...
            <p className="text-xs text-slate-400">自 A7R V 开始搭载独立 AI 芯片。不再仅仅识别“眼睛”，而是构建了人体/物体的骨架模型，理解物体的形态。</p>
         </div>
      </div>
   </div>
);
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
               ...
               <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs text-slate-400">
                     <thead className="bg-slate-800 text-slate-200 uppercase"><tr><th className="p-3">名称</th><th className="p-3">特点</th><th className="p-3">应用场景</th></tr></thead>
                     <tbody className="divide-y divide-slate-800">
                        <tr><td className="p-3 font-bold text-white">S-Log3</td><td className="p-3">14+ 档动态范围，暗部噪点略多，需向右曝光 (+1.7 EV)。</td><td className="p-3">电影制作，需后期加载 LUT。</td></tr>
                        <tr><td className="p-3 font-bold text-white">S-Cinetone</td><td className="p-3">源自 Venice 电影机，肤色红润，高光柔和。</td><td className="p-3">直出片，Vlog，快速交付项目。</td></tr>
                        <tr><td className="p-3 font-bold text-white">HLG</td><td className="p-3">即时 HDR，高动态范围，无需复杂调色。</td><td className="p-3">电视广播，HDR 内容制作。</td></tr>
                     </tbody>
                  </table>
               </div>
            </div>
            <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 flex flex-col">
               ...
               <div className="flex-1 bg-slate-800/50 rounded-lg p-4 border border-slate-700/50">
                  {format === 'HS' && <div className="animate-in fade-in slide-in-from-right-4"><div className="flex items-center gap-2 mb-2"><span className="bg-green-900/50 text-green-400 border border-green-500/30 px-2 py-0.5 rounded text-[10px] font-bold">High Efficiency</span><span className="text-xs font-bold text-slate-300">Long GOP</span></div><ul className="space-y-3 text-xs text-slate-400"><li><strong className="text-white">XAVC HS 8K:</strong> 记录 8K 影像。利用 HEVC 的高压缩率，在较小码率下实现 8K 录制。</li><li><strong className="text-white">XAVC HS 4K:</strong> 记录 4K 影像。相同画质下，文件体积比 XAVC S 小很多。</li><li className="bg-black/20 p-2 rounded text-[10px] border-l-2 border-green-500"><strong className="text-green-400 block mb-1">特点:</strong> 采用 H.265 (HEVC) 编码。计算量巨大，对电脑剪辑性能要求极高（建议使用代理）。但在相同数据量下画质更好。</li></ul></div>}
                  {format === 'S' && <div className="animate-in fade-in slide-in-from-right-4"><div className="flex items-center gap-2 mb-2"><span className="bg-blue-900/50 text-blue-400 border border-blue-500/30 px-2 py-0.5 rounded text-[10px] font-bold">Standard</span><span className="text-xs font-bold text-slate-300">Long GOP</span></div><ul className="space-y-3 text-xs text-slate-400"><li><strong className="text-white">XAVC S 4K:</strong> 以 3840×2160 分辨率记录。兼容性最好的格式。</li><li><strong className="text-white">XAVC S HD:</strong> 以 1920×1080 分辨率记录。</li><li className="bg-black/20 p-2 rounded text-[10px] border-l-2 border-blue-500"><strong className="text-blue-400 block mb-1">特点:</strong> 采用 H.264 (AVC) 编码。所有播放器都能播，剪辑压力适中。文件体积和画质的平衡点。</li></ul></div>}
                  {format === 'SI' && <div className="animate-in fade-in slide-in-from-right-4"><div className="flex items-center gap-2 mb-2"><span className="bg-orange-900/50 text-orange-400 border border-orange-500/30 px-2 py-0.5 rounded text-[10px] font-bold">Intra-frame</span><span className="text-xs font-bold text-slate-300">All-I</span></div><ul className="space-y-3 text-xs text-slate-400"><li><strong className="text-white">XAVC S-I 4K / HD:</strong> 每一帧都是独立压缩，不依赖前后帧（Intra）。</li><li className="bg-black/20 p-2 rounded text-[10px] border-l-2 border-orange-500"><strong className="text-orange-400 block mb-1">特点:</strong> 码率最高（可达 600Mbps+），文件巨大（需 V90 或 CFA 卡）。但剪辑极其流畅，不需要电脑进行复杂的解码运算。画质最佳，运动画面无伪影。</li></ul></div>}
               </div>
            </div>
         </div>
         <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-700">
               <table className="w-full text-left text-xs">
                  <thead className="bg-slate-800 text-slate-200 font-bold uppercase tracking-wider"><tr><th className="p-3 border-b border-slate-700">分辨率</th><th className="p-3 border-b border-slate-700">编码格式</th><th className="p-3 border-b border-slate-700">采样 / 位深</th><th className="p-3 border-b border-slate-700">帧率</th><th className="p-3 border-b border-slate-700 text-right">预计码率 (Mbps)</th></tr></thead>
                  <tbody className="divide-y divide-slate-800 bg-slate-900/50 text-slate-300">
                     <tr className="hover:bg-slate-800/50 transition-colors"><td className="p-3 font-mono text-white">4K UHD</td><td className="p-3"><span className="text-orange-400 font-bold">XAVC S-I</span></td><td className="p-3 text-cyan-400">4:2:2 10-bit</td><td className="p-3">60p</td><td className="p-3 text-right font-mono">600</td></tr>
                     <tr className="hover:bg-slate-800/50 transition-colors"><td className="p-3 font-mono text-white">4K UHD</td><td className="p-3"><span className="text-blue-400 font-bold">XAVC S</span></td><td className="p-3 text-cyan-400">4:2:2 10-bit</td><td className="p-3">120p (100p)</td><td className="p-3 text-right font-mono">280</td></tr>
                     <tr className="hover:bg-slate-800/50 transition-colors"><td className="p-3 font-mono text-white">4K UHD</td><td className="p-3"><span className="text-blue-400 font-bold">XAVC S</span></td><td className="p-3 text-cyan-400">4:2:2 10-bit</td><td className="p-3">60p (50p)</td><td className="p-3 text-right font-mono">200</td></tr>
                     <tr className="hover:bg-slate-800/50 transition-colors"><td className="p-3 font-mono text-white">4K UHD</td><td className="p-3"><span className="text-green-400 font-bold">XAVC HS</span></td><td className="p-3 text-slate-500">4:2:0 10-bit</td><td className="p-3">120p (100p)</td><td className="p-3 text-right font-mono">200</td></tr>
                     <tr className="hover:bg-slate-800/50 transition-colors"><td className="p-3 font-mono text-white">4K UHD</td><td className="p-3"><span className="text-blue-400 font-bold">XAVC S</span></td><td className="p-3 text-cyan-400">4:2:2 10-bit</td><td className="p-3">30p / 24p</td><td className="p-3 text-right font-mono">140 / 100</td></tr>
                     <tr className="hover:bg-slate-800/50 transition-colors"><td className="p-3 font-mono text-white">4K UHD</td><td className="p-3"><span className="text-blue-400 font-bold">XAVC S</span></td><td className="p-3 text-slate-500">4:2:0 8-bit</td><td className="p-3">120p</td><td className="p-3 text-right font-mono">200</td></tr>
                     <tr className="hover:bg-slate-800/50 transition-colors"><td className="p-3 font-mono text-white">4K UHD</td><td className="p-3"><span className="text-blue-400 font-bold">XAVC S</span></td><td className="p-3 text-slate-500">4:2:0 8-bit</td><td className="p-3">30p / 24p</td><td className="p-3 text-right font-mono">100 / 60</td></tr>
                  </tbody>
               </table>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
            ...
            <ul className="space-y-4 text-xs text-slate-400">
               <li className="border-l-2 border-yellow-500 pl-3"><strong className="text-white block mb-1">IBIS (五轴防抖)</strong>机身传感器物理位移。Sony 标称 5.5级 或 8.0级。</li>
               <li className="border-l-2 border-yellow-500 pl-3"><strong className="text-white block mb-1">Active Mode (增强防抖)</strong>数码裁切防抖。利用陀螺仪数据裁切画面边缘（约 1.1x），实现类似云台的稳定性。</li>
            </ul>
         </div>
         <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Zap size={18} className="text-cyan-500"/> 快门与马达</h3>
            <ul className="space-y-4 text-xs text-slate-400">
               <li className="border-l-2 border-cyan-500 pl-3"><strong className="text-white block mb-1">XD Linear Motor</strong>极高推力的线性马达。专为现代微单设计，静音、极速，能跟上 30fps 连拍。</li>
               <li className="border-l-2 border-cyan-500 pl-3"><strong className="text-white block mb-1">Global Shutter (a9 III)</strong>全域快门。所有像素同一时刻曝光。彻底消灭果冻效应，闪光灯同步无限制。</li>
            </ul>
         </div>
      </div>
   ...
   return (
      <div className="flex flex-col lg:flex-row h-full">
         <div className="flex-1 bg-black flex items-center justify-center p-8 relative">
            ...
            <div className="w-full max-w-4xl aspect-[3/2] bg-[#1a1a1a] border-4 border-slate-800 rounded-lg relative flex overflow-hidden font-sans select-none shadow-2xl">
               
               {/* Level 1: Main Categories */}
               <div className="w-[10%] min-w-[50px] bg-[#111] flex flex-col items-center py-4 gap-4 border-r border-[#333] shrink-0">
                  {menuStructure.map((m, i) => (
                     <div key={i} onClick={() => { setMainTab(i); setSubTab(0); }} className={`w-8 h-8 lg:w-10 lg:h-10 rounded flex items-center justify-center transition-all cursor-pointer ${mainTab === i ? m.color + ' text-white shadow-lg scale-110' : 'text-slate-500 hover:bg-slate-800'}`}>{m.icon}</div>
                  ))}
               </div>

               {/* Level 2: Sub Categories */}
               <div className="w-[30%] bg-[#1a1a1a] flex flex-col py-2 border-r border-[#333] shrink-0 overflow-y-auto">
                  <div className="px-4 py-2 text-[10px] lg:text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 border-b border-[#333] truncate">{currentMenu.name}</div>
               ...
               {/* Level 3: Items */}
               <div className="flex-1 bg-[#222] p-4 flex flex-col min-w-0">
                  <div className="text-lg lg:text-xl font-bold text-white mb-6 border-b border-slate-600 pb-2 flex items-center gap-2 truncate">
                     {currentSub.title}
                  </div>
                     ...
                     {currentSub.items.length > 0 ? (
                        currentSub.items.map((item, i) => (
                           <div key={i} className="flex justify-between items-center p-3 bg-[#333] rounded hover:bg-[#444] cursor-pointer group transition-colors">
                              <span className="text-xs lg:text-sm text-slate-200 font-medium truncate mr-2">{item.n}</span>
                              <span className="text-[10px] lg:text-xs font-bold text-[#fcd34d] group-hover:text-white px-2 py-1 rounded bg-[#000]/20 shrink-0">{item.v}</span>
                           </div>
                        ))
                     ) : <div className="text-slate-500 text-xs italic p-4">No settings available.</div>}
                  </div>
                  <div className="mt-auto pt-4 flex justify-between items-end">
                     <div className="text-[10px] text-slate-500">* Interactive Mockup</div>
         ...
         <div className="w-full lg:w-96 bg-slate-900 border-l border-slate-800 p-6 flex flex-col overflow-y-auto">
            ...
            <div className="space-y-6">
               <div className="bg-slate-800 p-4 rounded border border-slate-700">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">索尼旧菜单（水平Tab）被诟病多年。新菜单（a7S3起）采用了垂直三级结构：<br/><br/>1. <span className="text-red-400">主分类</span> (左侧图标)<br/>2. <span className="text-yellow-400">子分类</span> (中间列表)<br/>3. <span className="text-white">具体设置</span> (右侧详情)<br/><br/>这大大减少了翻页寻找功能的痛苦。</p>
               </div>
               <div className="bg-slate-800 p-4 rounded border border-slate-700">
                  <div className="font-bold text-white text-sm mb-2">防抖设置 (Stabilization)</div>
                  <ul className="text-xs text-slate-400 space-y-2"><li><strong className="text-red-400">Shooting &gt; Image Stabilization</strong></li><li className="pl-2 border-l-2 border-slate-600"><span className="text-white">SteadyShot:</span> 开启/关闭</li><li className="pl-2 border-l-2 border-slate-600"><span className="text-white">SteadyShot Mode:</span> <br/> - <span className="text-yellow-400">Standard (标准):</span> 纯物理防抖<br/> - <span className="text-yellow-400">Active (增强):</span> 物理+电子裁切 (适合手持走动)</li><li className="pl-2 border-l-2 border-slate-600"><span className="text-white">Adjust. (手动焦距):</span> <br/>使用老镜头时必须手动输入焦距，否则防抖会失效或产生果冻。</li></ul>
               </div>
            </div>
         </div>
      </div>
